...
export function Footer() {
  ...
  return (
    <footer className="relative py-16 px-4 md:px-8 bg-black border-t border-white/10 overflow-hidden">
          ...
          >
            {footerLinks.map((link, index) => (
              <motion.a
                key={index}
                href={link.href}
                className="text-gray-400 hover:text-white tracking-wider uppercase text-sm transition-colors duration-300 relative group"
                initial={{ opacity: 0, y: 20 }}
                whileInView={{ opacity: 1, y: 0 }}
                viewport={{ once: false }}
            ...
            {socialLinks.map((social, index) => {
              ...
              return (
                <motion.a
                  key={index}
                  href={social.href}
                  className="w-10 h-10 border border-white/20 flex items-center justify-center text-gray-400 hover:text-white hover:border-white/40 transition-all duration-300"
                  initial={{ opacity: 0, scale: 0 }}
                  whileInView={{ opacity: 1, scale: 1 }}
                  viewport={{ once: false }}
      ...
      <motion.button
        className="fixed bottom-4 right-4 md:bottom-8 md:right-8 w-12 h-12 md:w-14 md:h-14 bg-white text-black flex items-center justify-center group hover:bg-gradient-to-br hover:from-zinc-800 hover:to-black hover:text-white transition-all duration-500 z-50"
        onClick={scrollToTop}
